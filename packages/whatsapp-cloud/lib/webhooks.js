...
function verifySignature(req, secret) {
  ...
  const expected = crypto
    ...
    .digest('hex')
  if (expected !== actual) {
    throw new HttpError(400, 'Invalid \'x-hub-signature-256\'.', `Expected 'x-hub-signature-256': '${expected}'.`)
  }
}
 ...
 *    The notification values parsed from the request.
 */
function parse(req) {
  const body = req.body
