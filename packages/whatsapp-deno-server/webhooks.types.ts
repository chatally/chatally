...
export function getChanges(notification: Notification): Change[] {
  return notification.entry?.flatMap((entry) => entry.changes) || [];
}

export function getValues(notification: Notification): Value[] {
  return getChanges(notification).map((change) => change.value);
}
...
export function getMessages(notification: Notification): IncomingMessage[] {
  return getValues(notification).flatMap((value) => value.messages ?? []);
}

export function getStatuses(notification: Notification): Status[] {
  return getValues(notification).flatMap((value) => value.statuses ?? []);
}
...
/**
 * Handle individual WhatsApp webhook Messages.
 */
...
/**
 * Handle individual WhatsApp webhook status updates.
 */
...
export interface WebhooksConfig {
  /** In development secret and verifyToken are optional */
  isDevelopment?: boolean;

  /** SHA-256 secret to validate incoming payloads, mandatory in production */
  secret?: string;

  /**
   * Shared secret token to verify the webhook during registration,
